...
import sys
import argparse
# import qua_resnet_pytorch as resnet
import model.resnet_fs_quantiable as resnet
# # Setup warnings
import warnings
import model.common as utils

parser = argparse.ArgumentParser()
parser.add_argument('-a', '--architecture', default="resnet18", type=str,
                    help='which ResNet archtecture need to be quantized(default:resnet18)')
...
sketch_rate = '['+((modelname.split("_resnet50_")[1]).split(".p")[0])+']*16'
...
def load_model(model_file):
    model = resnet.resnet('resnet50', sketch_rate=utils.get_sketch_rate(sketch_rate), start_conv=1).to('cpu')
...
myModel = resnet.resnet('resnet50', sketch_rate=utils.get_sketch_rate(sketch_rate), start_conv=1).to('cpu')
ckpt = torch.load('/nobackup/sc19yt/project/pytorch_model_zoo_pretrain_model/pruning_models/' + modelname + '.pt', map_location='cpu')
myModel.load_state_dict(ckpt['state_dict'])
